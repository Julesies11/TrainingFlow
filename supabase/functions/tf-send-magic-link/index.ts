// supabase/functions/tf-send-magic-link/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    // 1. Create a service role client to generate the login link
    ...
    // 2. Generate the magic link securely
    ...
    const customActionLink = `${clientOrigin}/auth/callback?token_hash=${hashedToken}&type=magiclink`;
    ...
    if (!resendApiKey) {
      ...
      console.log(`[TESTING] Magic link for ${email}: ${customActionLink}`);
      
      return new Response(
        JSON.stringify({ 
          success: true, 
          message: "Magic link generated but TF_RESEND_API_KEY is not configured. Link logged to Deno console logs for testing." 
        }),
    ...
    const resendResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [email],
        subject: "Sign in to TrainingFlow",
        html: `
          <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 500px; margin: 40px auto; padding: 32px; border: 1px solid #e5e7eb; border-radius: 16px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);">
            ...
            <h2 style="font-size: 20px; font-weight: 700; color: #111827; margin-bottom: 12px; text-align: center;">Welcome back!</h2>
            <p style="font-size: 14px; color: #4b5563; line-height: 1.6; margin-bottom: 24px; text-align: center;">
              Click the button below to securely sign in to your TrainingFlow account. This link will expire in 1 hour.
            </p>
    ...
    if (!resendResponse.ok) {
      ...
      return new Response(
        JSON.stringify({ error: resendError.message || "Failed to deliver magic link email" }),
        { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    console.error(`[tf-send-magic-link Error]`, error);
    return new Response(
